"""ECAPA-TDNN with WavLM-Large feature extractor — vendored from
https://github.com/microsoft/UniSpeech/tree/main/downstreams/speaker_verification

Compared to upstream:
  - removed the ~300 lines of commented-out duplicate code at the top
  - the hard-coded `S3PRL_CACHE_DIR` path is now optional: we only
    override `torch.hub.set_dir` when the env var is set.

Part of the architecture is borrowed from
...
class AttentiveStatsPool(nn.Module):
    ...
    def forward(self, x):
        ...
        # tanh activation here trains better than relu in practice.
    ...
    def __init__(self, feat_dim=80, channels=512, emb_dim=192, global_context_att=False,
        ...
        if feat_type == "fbank":
            self.feature_extract = trans.MelSpectrogram(
                sample_rate=sr, n_fft=512, win_length=win_len, hop_length=hop_len,
                f_min=0.0, f_max=sr // 2, pad=0, n_mels=feat_dim,
            )
        elif feat_type == "mfcc":
            ...
            # Talker-T2AV uses feat_type="wavlm_large" — fetch the s3prl WavLM
            # upstream via torch.hub. Defaults to torch's own cache (~/.cache/torch/hub).
            torch.hub._validate_not_a_forked_repo = lambda a, b, c: True
            _cache = os.environ.get("S3PRL_CACHE_DIR")
            if _cache:
                torch.hub.set_dir(_cache)
            self.feature_extract = torch.hub.load("s3prl/s3prl", feat_type)

            n_layers = len(self.feature_extract.model.encoder.layers)
            for i in (11, 23):
                if n_layers == 24 and hasattr(self.feature_extract.model.encoder.layers[i].self_attn, "fp32_attention"):
                    self.feature_extract.model.encoder.layers[i].self_attn.fp32_attention = False
