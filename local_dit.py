...
import math
from llama4nar import ContinuousDecoder, DecoderConfig  # 你自己那份 NAR 实现
...
class VoxCPMLocDiT(nn.Module):
    ...
    def __init__(
        ...
        self.global_cond_proj = nn.Linear(global_cond_channels, config.hidden_size, bias=True)
        ...
        self.decoder = ContinuousDecoder(config)
    ...
    def forward(
        ...
        global_cond: (N, global_cond_channels) tensor of global condition (spk or first_motion)
        ...
        global_cond = self.global_cond_proj(global_cond.unsqueeze(1).contiguous())
        prefix = cond.size(1)
        ...
        x = torch.cat([(mu + t).unsqueeze(1), global_cond, cond, x], dim=1)
        hidden = self.decoder(x)
        hidden = hidden[:, prefix + 2 :, :]
