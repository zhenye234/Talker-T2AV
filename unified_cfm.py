...
class CfmConfig(BaseModel):
    ...
    training_cond_drop_rate: float = 0.05
...
class UnifiedCFM(torch.nn.Module):
    def __init__(
        ...
        self.training_cond_drop_rate = cfm_params.training_cond_drop_rate
    ...
    def solve_euler(
        ...
        for step in range(1, len(t_span)):
            if use_cfg_zero_star and step <= zero_init_steps:
                ...
                gc_in = torch.zeros([2 * b, global_cond.size(1)], device=x.device, dtype=x.dtype)
                x_in[:b], x_in[b:] = x, x
                mu_in[:b] = mu
                ...
                gc_in[:b], gc_in[b:] = global_cond, global_cond

                dphi_dt = self.estimator(x_in, mu_in, t_in, cond_in, gc_in, dt_in)
    ...
    def compute_loss(
        ...
        if self.training_cond_drop_rate > 0:
            cond_mask = torch.rand(b, device=x1.device) > self.training_cond_drop_rate
            cond = cond * cond_mask.view(-1, 1, 1)
        ...
        def model_fn(z_sample, r_sample, t_sample):
            return self.estimator(z_sample, mu, t_sample, cond, global_cond, dt=t_sample - r_sample)
